...
-- Begin additional modifications, joins --
  ...
  , combine_ipeds AS (
    SELECT sfdc.*, cs.*, SPLIT(academic_calendar_value_c, ' ')[OFFSET(0)] AS college_academic_calendar -- extract first word of the STRING
    FROM
        sfdc_ipeds             AS sfdc
  ...
  , prep_nsc AS ( -- filter for graduates only; select certain fields, modify fields, filter to matches
    ...
    WHERE
      ...
      AND ((Degree_Title LIKE '%BACHELOR%' OR Degree_Title LIKE '%BS%' OR Degree_Title LIKE '%BA%' OR Degree_Title LIKE '%AB%') OR
  ...
  , nsc_to_pat_data
     ...
     , align_acdemic_calendars AS ( -- create PAT records where GAS academic calendar aligns with college academic calendar
        ...
        WHERE
            (College_Track_Status_Name = 'Active: Post-Secondary' AND GAS_calendar = college_academic_calendar) -- if student is currently active, align college academic calendar more precisely to GAS academic calendar
           OR (College_Track_Status_Name <> 'Active: Post-Secondary' AND GAS_calendar = college_academic_calendar OR college_academic_calendar = "Semester" AND AT_Name LIKE "%Quarter%") -- if student is not active, less precise is ok; if there is a PAT record that doesn't perfectly line up with the academic calendar, that is ok. Just push it into the existing record
        )
