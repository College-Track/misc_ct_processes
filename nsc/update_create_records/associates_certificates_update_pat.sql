-- Identify students who earned an Associate's degree or certificate
-- Students with an existing PAT to store data
...
WITH
  ...
  , sfdc AS (
    ...
    WHERE
      ...
      AND type_of_degree_earned_c IS NULL -- BA/BS, AA/AS, or Certificate not recorded on this PAT
...
-- Begin additional modifications, joins --
  ...
  , prep_nsc AS ( -- filter for graduates only; select certain fields, modify fields, filter to matches
    ...
    WHERE
      ...
      AND _2_year___4_year IN ('4', '2', 'L') -- graduated from 4-year, 2-year, "L" for certificate and other; keep 4-year as they still offer AAs/Certificates
      ...
      AND ((Degree_Title LIKE '%ASSOCIATE%' OR Degree_Title LIKE '%CERTIF%' OR Degree_Title LIKE '%AA %' OR
            Degree_Title LIKE '%AS %' OR Degree_Title LIKE '%CA %') OR
           Degree_Title IS NULL) -- student earned a AA/CERT OR it is not listed (vs. listing pre-nursing or certificate)
    )
  ...
  , nsc_to_pat_data
        AS ( -- majors + map nsc grad data (with GAS term) to graduating PAT record, use this to identify PATs that need to be created in next CTE
        SELECT DISTINCT
            -- id fields
            pat.Contact_Id
          , pat.AT_Id
          ...
          , CASE
                WHEN Degree_Title LIKE "%CERTIF%"
                    THEN 'Certificate'
                    ELSE '2-year degree'
                END                    AS type_of_degree_earned_c
          , pat.AT_Name
          , pat.GAS_Name
          , nsc.GAS_calendar
  ...
  , align_acdemic_calendars
        AS ( -- create PAT records where GAS academic calendar aligns with college academic calendar
        SELECT *
        FROM nsc_to_pat_data
        WHERE
             college_academic_calendar = "Semester" AND AT_Name LIKE "%Quarter%" -- if there is a PAT record that doesn't perfectly line up with the academic calendar, that is ok. Just push it into the existing record
          OR GAS_calendar = college_academic_calendar
        )
  ...
  , rank
        ...
        SELECT
            * EXCEPT (Degree_Major_1, Degree_Title, type_of_degree_earned_c, sfdc_major)

            -- calculate ranking
          , DENSE_RANK() OVER (PARTITION BY AT_Id ORDER BY type_of_degree_earned_c ASC) AS row_number_degree --ASC (rank 2-year above Certificate)

          -- Concatenate list of degrees and majors from nsc, and for salesforce, into a single row
          , STRING_AGG(type_of_degree_earned_c, "|") OVER (PARTITION BY AT_Id )         AS partition_sfdc_degree -- concatenate degrees if student earned more than 1
          , STRING_AGG(sfdc_major, "|") OVER (PARTITION BY AT_Id)                       AS partition_sfdc_major -- concatenate majors if student majored in more than 1
          , STRING_AGG(Degree_Title, "|") OVER (PARTITION BY AT_Id)                     AS partition_nsc_degree -- concatenate list of degrees into single row based on PAT record
          , STRING_AGG(Degree_Major_1, "|") OVER (PARTITION BY AT_Id)                   AS partition_nsc_major -- concatenate list of majors into single row based on PAT record
        FROM school_for_pat

        QUALIFY
            (DENSE_RANK() OVER (PARTITION BY AT_Id ORDER BY type_of_degree_earned_c ASC) = 1 -- row_number_degree; take highest ranked degree
                )
        )
  ...
  , for_prod AS ( -- add degree to PAT record
    SELECT DISTINCT
      ...
      , AT_Name
      , PAT_GAS_id
      , partition_sfdc_degree AS list_sfdc_degrees
      ...
      , CASE
          WHEN INSTR(partition_sfdc_degree, '|') > 0
            ...
            END               AS type_of_degree_earned_c -- if there is a comma "," then extract first string, otherwise just return the value     AS Type_of_Degree_Earned__c
    FROM rank
    )
...
/*
Salesforce list of majors:

Undeclared
Arts: Design, Performing, or Visual
Biological, Environmental, or Agricultural Sciences
Business
...
Health Sciences
Liberal Arts or History
Literature or Languages
Math or Statistics
Psychology
Sociology, Political Science, Economics, or other Social Sciences
 */
